...
#### As we saw eariler in the course, Python doesnt require the explicit declartion of types.
### Nevertheless, its good to practice need to ensure all of your variables are of the right type.
...
##### You may already see that number = input("Number: )" returns a string, not an int.
#### But meow will likely want an int!
...
##### Notice, though, that out program still throws an error.

#### After installing mypy, execute mypy type_hints.py in the terminal window.
### mypy will provide some guidance about how to fix this error.
...
##### Again, executing mypy type_hints.py in the terminal window provides much more specific feedback to you, the programmer.
...
###### Lets introduce a new error by assuming that meow will return to us a string or str.
